...
int DeleteMax(Heap* heap){
	...
	int i, next;
	for(i = 1;i * 2 <= heap->Size;i = next){
		next = i * 2;
		if(next < heap->Size && heap->Elements[next + 1] > heap->Elements[next]){
			next += 1;
		}
		if(lastElement < heap->Elements[next]){
			heap->Elements[i] = heap->Elements[next];
		}else{
			break;
		}
	}
...
int* GetElements(Heap* heap){
	...
	//Get elements.
	int* getElements = (int*)malloc(sizeof(int) * heap->Size);
	for(int i = 0;i < heap->Size;++i){
		getElements[i] = heap->Elements[i + 1];
	}
	return getElements;
}
